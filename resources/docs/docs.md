...
The `home-routes` handler in the `twitter-port.routes.home` namespace
...
The `home-page` function will in turn call the `twitter-port.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `twitter-port.handler` namespace:
...
Request middleware functions are located under the `twitter-port.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `twitter-port.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `twitter-port.db.core` in some other namespace.
